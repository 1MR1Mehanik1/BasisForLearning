...
public class ItemNBTHelper {
    ...
    // SET //-----------------
    public static void setBoolean(ItemStack stack, String tag, boolean b) { getNBT(stack).setBoolean(tag, b); }
    ...
    public static void setInt(ItemStack stack, String tag, int i) { getNBT(stack).setInteger(tag, i); }
    ...
    public static void setCompound(ItemStack stack, String tag, NBTTagCompound cmp) { if(!tag.equalsIgnoreCase("ench")) getNBT(stack).setTag(tag, cmp); } // not override the enchantments
    public static void setString(ItemStack stack, String tag, String s) { getNBT(stack).setString(tag, s); }
    ...
    public static boolean getBoolean(ItemStack stack, String tag, boolean defaultExpected) { return verifyExistance(stack, tag) ? getNBT(stack).getBoolean(tag) : defaultExpected; }
    ...
    public static int getInt(ItemStack stack, String tag, int defaultExpected) { return verifyExistance(stack, tag) ? getNBT(stack).getInteger(tag) : defaultExpected; }
    ...
    public static boolean matchTag(@Nullable NBTBase template, @Nullable NBTBase target) {
        if(template instanceof NBTTagCompound && target instanceof NBTTagCompound) return matchTagCompound((NBTTagCompound) template, (NBTTagCompound) target);
        else if(template instanceof NBTTagList && target instanceof NBTTagList) return matchTagList((NBTTagList) template, (NBTTagList) target);
        else return template == null || (target != null && target.equals(template));
    }
    ...
    private static boolean matchTagCompound(NBTTagCompound template, NBTTagCompound target) {
        ...
        for(String key : template.getKeySet()) if (!matchTag(template.getTag(key), target.getTag(key))) return false;
        return true;
    }
    ...
    private static boolean matchTagList(NBTTagList template, NBTTagList target) {
        ...
        for (int i = 0; i < template.tagCount(); i++) if (!matchTag(template.get(i), target.get(i))) return false;
        return true;
    }
}
